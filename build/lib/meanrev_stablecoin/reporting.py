...
本项目按两份 TeX 方法论建立了只读原始数据审计、精确时间匹配、非参数漂移、OU/阈值 OU、CIR、仿射变换、MOU/MCIR、半参数多滞后 copula、边界 bootstrap、滚动预测、脱锚事件和首次回复时间流程。只有 `pytest` 与一键流程的最终状态均为 PASS 时，manifest 才会标记项目完成。
