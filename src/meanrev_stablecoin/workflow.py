...
def run_full_model_suite(config: dict[str, Any], force: bool = False, dry_run: bool = False) -> dict[str, Any]:
    ...
    logger = setup_logging("14_full_model_suite", config)
    target = _paths(config)["tables"] / "updated_stage_gate.csv"
    if dry_run:
        logger.info("DRY RUN full model suite")
        return {"dry_run": True}
    if target.exists() and not force:
        gates = pd.read_csv(target)
        logger.info("Using cached full model suite: %s", target)
        return {"cached": True, "stage_gate_pass": bool(gates["pass"].all()), "path": str(target)}
    baseline = select_sample(_load_prepared(config), "baseline")
    result = run_full_suite(config, baseline, _paths(config)["tables"], _paths(config)["models"])
    logger.info(
        "Full model suite complete registry=%d covered=%d copulas=%d selected=%s gates=%s",
        result["registry_models"], result["registry_covered"], result["copula_models"],
        result["selected_validation_model"], result["stage_gate_pass"],
    )
    if not result["stage_gate_pass"]:
        raise RuntimeError("Updated full-model stage gate failed; inspect output/tables/updated_stage_gate.csv")
    return result
